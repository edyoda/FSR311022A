...
$(document).ready(function () {
  function onCardClick(e) {
    // Remove the border from older cards and add it to the current card
    console.log("Card clicked", e.currentTarget.id);
    ...
    for (let i = 0; i < videoPlaySectionData.length; i++) {
      if (videoPlaySectionData[i].id === id) {
        console.log("ID match found", videoPlaySectionData[i]);
        videoData = videoPlaySectionData[i];
        // Since we found the item we were looking for, no need to run this loop further now
        break;
      }
    }
    $("#video-player").attr(
  ...
  for (let i = 0; i < playlistData.length; i++) {
    // const playlistCardData = playlistData[i];
    const card = createPlaylistCard(playlistData[i]);
    $("#playlist-wrapper").append(card);
  }
  // Put the first video data into the left section
});
